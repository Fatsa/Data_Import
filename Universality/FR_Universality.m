...
function [FR_Data] = FR_Universality(fileName, pathName)
    % Initialization    
    ...
    % start resize the data_Frequency into FR_Data
    lengths = size (data_Frequency);
    FR_Data = zeros (lengths(1),7);
    
    % Arousal
    FR_Data(:,1) = data_Frequency (:, 9);
    FR_Data(:,2) = data_Frequency (:, 8);   
    FR_Data(:,3) = data_Frequency (:, 6);
    FR_Data(:,4) = data_Frequency (:, 4);
    FR_Data(:,5) = data_Frequency (:, 2);
    FR_Data(:,6) = data_Frequency (:, 3);
    FR_Data(:,7) = data_Frequency (:, 7);
end
